...
struct server_slot {
    ...
    visible_image_key live_text_images;
...
static slot_reuse slot_probe_reuse_locked(server *s, server_slot *slot,
    ...
    {
        visible_image_key images;
        char *key = visible_prompt_key(req, ptext, &images);
        bool ok = key &&
            slot->live_text_images.count == ds4_session_vision_image_count(slot->session) &&
            visible_image_prefix_matches(&images, &slot->live_text_images,
                                         slot->live_text_len) &&
            byte_prefix_match(key, plen, slot->live_text, slot->live_text_len);
        free(key);
        if (ok) {
 ...
 * tool_mu. */
static void slot_refresh_live_text(server *s, server_slot *slot, const request *req) {
    if (!slot) return;
    free(slot->live_text);
    slot->live_text = NULL;
    slot->live_text_len = 0;
    slot->live_text_pos = 0;
    memset(&slot->live_text_images, 0, sizeof(slot->live_text_images));
    if (!s || !slot->session ||
    ...
    } else if (req && count <= req->image_count) {
        /* Normalize only actual historical image blocks. The probe can then
         * compare fresh request nonces without detokenizing under tool_mu.
         * The request is still owned by this job until job_complete(). */
        char *text = render_live_prompt_text(s, slot, req, &slot->live_text_len);
        if (text) {
            request history = *req;
            history.image_count = count;
            slot->live_text = visible_prompt_key(&history, text, &slot->live_text_images);
            free(text);
        }
    }
...
static void *worker_main(void *arg) {
    ...
    for (;;) {
        ...
        slot_refresh_live_text(s, &s->slots[0], &j->req);
...
static void *slot_worker_main(void *arg) {
    ...
    for (;;) {
        ...
        slot_refresh_live_text(s, slot, &j->req);
...
static void test_slot_probe_vision_tiers(void) {
     ...
     *    full re-prefill instead of prefix reuse). */
    {
        ...
        strcpy(j.req.image_markers[0], "<M1>");
     ...
     *    prefill (changed-image class). */
    {
        ...
        live_img.token_start = 2;
        live_img.embedding.token_count = 2;
        live_img.embedding.fingerprint[0] = 7;
        ds4_session_set_test_images(slots[0].session, &live_img, 1);
        slots[0].live_text = xstrdup("hi \036\036\036\036 world");
        slots[0].live_text_len = strlen(slots[0].live_text);
        slots[0].live_text_images.count = 1;
        slots[0].live_text_images.offsets[0] = 3;
        slots[0].live_text_pos = 10;
        slots[1].session = ds4_session_new_test_checkpoint(NULL, 0);
        job j = {0};
        ds4_tokens_push(&j.req.prompt, 999);
        j.req.prompt_text = xstrdup("hi <M1> world next");
        j.req.image_markers = xmalloc(sizeof(*j.req.image_markers));
        strcpy(j.req.image_markers[0], "<M1>");
        j.req.images = calloc(1, sizeof(*j.req.images));
        j.req.images[0].token_start = 4; /* re-tokenization shifted the image */
        j.req.images[0].embedding.token_count = 2;
        ...
        TEST_ASSERT(pr.kind == REUSE_MEMORY_TEXT);
        TEST_ASSERT(pr.reuse_tokens == 10);
        /* Equal normalized bytes alone cannot authenticate a relocated image:
         * literal marker-mask bytes are not image conditioning. */
        free(j.req.prompt_text);
        j.req.prompt_text = xstrdup("hi \036\036\036\036 world <M1>");
        TEST_ASSERT(slot_probe_reuse_locked(&s, &slots[0], &j.req).kind == REUSE_NONE);
        ds4_session_free_test_checkpoint(slots[0].session);
        ds4_session_free_test_checkpoint(slots[1].session);
        free(slots[0].live_text);
        request_free(&j.req);
